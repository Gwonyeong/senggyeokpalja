...
export function determinePaljaType(sajuData) {
    ...
    const axis4 = determineLifestyle(sajuData.ohaeng);
...
function determinePerception(sibsin) {
    ...
    return idealScore > practicalScore ? 'G' : 'S';
}
...
function determineJudgement(sibsin) {
    ...
    return ruleScore > harmonyScore ? 'I' : 'H';
}

function determineLifestyle(ohaeng) {
...
const SINSAL_MAP = {
  // 도화살 (일간별 지지 대응)
  도화: {
    "甲": ["午"], "乙": ["午"], // 인오술 삼합 → 도화는 卯
    "丙": ["卯"], "丁": ["卯"], // 사유축 삼합 → 도화는 午
    "戊": ["卯"], "己": ["卯"], // 사유축 삼합 → 도화는 午
    "庚": ["子"], "辛": ["子"], // 신자진 삼합 → 도화는 酉
    "壬": ["酉"], "癸": ["酉"]  // 해묘미 삼합 → 도화는 子
  },
...
export function enrichSinsalData(sinsalData, sinsalJsonData = []) {
  ...
  Object.keys(sinsalData).forEach(sinsalName => {
    ...
    const sinsalInfo = sinsalJsonData.find(item =>
      item.name_kr === sinsalName ||
      item.name_kr.includes(sinsalName.replace('살', ''))
    );
    ...
    enriched[sinsalName] = {
      ...
      info: sinsalInfo || {
        ...
        expert_explanation: '해당 신살에 대한 상세 정보가 없습니다.'
      }
    };
  });

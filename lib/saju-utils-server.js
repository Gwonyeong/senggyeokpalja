...
export function calculateSajuForServer(birthDate, timeIndex, isLunar = false) {
  // timeIndex를 시간으로 변환
  let hour = 12; // 기본값: 오시(6) -> 12시 (noon)
  let minute = 30;

  // timeIndex에 따른 시간 매핑
  const timeMapping = {
    0: { hour: 0, minute: 30 }, // 자시: 23:30 ~ 01:29 -> 00:30
    1: { hour: 2, minute: 30 }, // 축시: 01:30 ~ 03:29 -> 02:30
    2: { hour: 4, minute: 30 }, // 인시: 03:30 ~ 05:29 -> 04:30
    3: { hour: 6, minute: 30 }, // 묘시: 05:30 ~ 07:29 -> 06:30
    4: { hour: 8, minute: 30 }, // 진시: 07:30 ~ 09:29 -> 08:30
    5: { hour: 10, minute: 30 }, // 사시: 09:30 ~ 11:29 -> 10:30
    6: { hour: 12, minute: 30 }, // 오시: 11:30 ~ 13:29 -> 12:30
    7: { hour: 14, minute: 30 }, // 미시: 13:30 ~ 15:29 -> 14:30
    8: { hour: 16, minute: 30 }, // 신시: 15:30 ~ 17:29 -> 16:30
    9: { hour: 18, minute: 30 }, // 유시: 17:30 ~ 19:29 -> 18:30
    10: { hour: 20, minute: 30 }, // 술시: 19:30 ~ 21:29 -> 20:30
    11: { hour: 22, minute: 30 }, // 해시: 21:30 ~ 23:29 -> 22:30
  };

  if (
    timeIndex !== undefined &&
    timeIndex !== "unknown" &&
    timeMapping[timeIndex]
  ) {
    hour = timeMapping[timeIndex].hour;
    minute = timeMapping[timeIndex].minute;
  }
  ...
  const convertPillar = (pillar, element, yinYang) => ({
    han:
      pillar.heavenlyStem === "갑"
        ? "甲"
        : pillar.heavenlyStem === "을"
        ? "乙"
        : pillar.heavenlyStem === "병"
        ? "丙"
        : pillar.heavenlyStem === "정"
        ? "丁"
        : pillar.heavenlyStem === "무"
        ? "戊"
        : pillar.heavenlyStem === "기"
        ? "己"
        : pillar.heavenlyStem === "경"
        ? "庚"
        : pillar.heavenlyStem === "신"
        ? "辛"
        : pillar.heavenlyStem === "임"
        ? "壬"
        : pillar.heavenlyStem === "계"
        ? "癸"
        : pillar.heavenlyStem,
  ...
  const convertBranch = (pillar, element, yinYang) => ({
    han:
      pillar.earthlyBranch === "자"
        ? "子"
        : pillar.earthlyBranch === "축"
        ? "丑"
        : pillar.earthlyBranch === "인"
        ? "寅"
        : pillar.earthlyBranch === "묘"
        ? "卯"
        : pillar.earthlyBranch === "진"
        ? "辰"
        : pillar.earthlyBranch === "사"
        ? "巳"
        : pillar.earthlyBranch === "오"
        ? "午"
        : pillar.earthlyBranch === "미"
        ? "未"
        : pillar.earthlyBranch === "신"
        ? "申"
        : pillar.earthlyBranch === "유"
        ? "酉"
        : pillar.earthlyBranch === "술"
        ? "戌"
        : pillar.earthlyBranch === "해"
        ? "亥"
  ...
  const palja = {
    yunju: {
      gan: convertPillar(
        fourPillars.year,
        fourPillars.yearElement,
        fourPillars.yearYinYang
      ),
      ji: convertBranch(
        fourPillars.year,
        fourPillars.yearElement,
    ...
    ilju: {
      gan: convertPillar(
        fourPillars.day,
        fourPillars.dayElement,
        fourPillars.dayYinYang
      ),
      ji: convertBranch(
        fourPillars.day,
        fourPillars.dayElement,
    ...
    siju: {
      gan: convertPillar(
        fourPillars.hour,
        fourPillars.hourElement,
        fourPillars.hourYinYang
      ),
      ji: convertBranch(
        fourPillars.hour,
        fourPillars.hourElement,
  ...
  // 천간 기반 십신 계산
  const calculateCheonganSibsin = (palja, ilgan) => {
    ...
    // 4개의 천간 (연간, 월간, 일간, 시간)에서 십신 계산
    // 일간은 자기 자신이므로 제외
    const ganList = [
      palja.yunju?.gan, // 연간
      palja.wolju?.gan, // 월간
      // palja.ilju?.gan은 일간이므로 제외
      palja.siju?.gan, // 시간
    ].filter((gan) => gan && gan.han);

    ganList.forEach((gan) => {
      const ganOhaeng = gan.ohaeng;
      const ganEumYang = gan.eumYang;
      ...
      // 일간과의 관계로 십신 결정
      if (ganOhaeng === ilganOhaeng) {
        // 같은 오행
        sibsinType = ganEumYang === ilganEumYang ? "비견" : "겁재";
      } else if (ohaengSaeng[ilganOhaeng] === ganOhaeng) {
        // 일간이 생하는 오행 (생출)
        sibsinType = ganEumYang === ilganEumYang ? "식신" : "상관";
      } else if (ohaengGeuk[ilganOhaeng] === ganOhaeng) {
        // 일간이 극하는 오행 (극출)
        sibsinType = ganEumYang === ilganEumYang ? "편재" : "정재";
      } else if (ohaengGeuk[ganOhaeng] === ilganOhaeng) {
        // 일간을 극하는 오행 (극입)
        sibsinType = ganEumYang === ilganEumYang ? "편관" : "정관";
      } else if (ohaengSaeng[ganOhaeng] === ilganOhaeng) {
        // 일간을 생하는 오행 (생입)
        sibsinType = ganEumYang === ilganEumYang ? "편인" : "정인";
      }
  ...
  const cheonganSibsinInfo = calculateCheonganSibsin(palja, ilgan);
  ...
  const findPrimarySibsin = (sibsin, ohaeng, ilgan) => {
    ...
    for (const [key, value] of Object.entries(sibsin)) {
      if (value > maxCount) {
        ...
        if (
          sibsinMeaning[key].priority < sibsinMeaning[primarySibsin].priority
        ) {
          primarySibsin = key;
        }
      }
    ...
    if (!primarySibsin || maxCount === 0) {
      const ohaengTotal = Object.values(ohaeng).reduce(
        (sum, val) => sum + val,
        0
      );
      const ilganOhaeng = ilgan.ohaeng;
    ...
    return {
      ...
      description: `${primarySibsin}(${
        sibsinMeaning[primarySibsin]?.meaning || ""
      })이 당신의 핵심 성향입니다.`,
    };
  };
  ...
  return {
    ...
    ilgan: ilgan,
    ohaeng: ohaengCount,
    sibsin: jijiSibsinInfo, // 지지 기반 십신
    cheonganSibsin: cheonganSibsinInfo, // 천간 기반 십신

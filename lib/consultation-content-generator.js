...
export const generateSection1Content = (consultation) => {
  ...
  const birthDateStr = new Date(birthDate).toLocaleDateString("ko-KR");
  const calendarType = lunarCalendar ? "음력" : "양력";
  const mbti = additionalData?.mbti || "정보 없음";
  const name = additionalData?.name || "고객";

  const speechBubbles = [
    {
      text: `안녕하세요, ${name}님!\n토리가 여러분의 운명을 해석해드릴게요.`,

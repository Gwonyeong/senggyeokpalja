 ...
 * @param {string} options.backgroundImage - 배경 이미지 경로
 * @param {Object} options.panelStyle - 패널 스타일 옵션
 * @param {Object} options.imageStyle - 이미지 스타일 옵션
 * @param {Array} options.speechBubbles - 사용자 정의 말풍선 배열
 * @param {Array} options.characterImages - 사용자 정의 캐릭터 이미지 배열
 */
export const generateSection1Content = (consultation, options = {}) => {
  // 기본 이미지 경로 또는 전달받은 이미지 사용
  const backgroundImage = options.backgroundImage || "/assets/images/results/section1/1.png";
  ...
  // 사용자 정의 이미지 스타일이 있으면 병합
  const imageStyle = { ...defaultImageStyle, ...options.imageStyle };
  ...
  // 사용자 정의 말풍선이 있으면 사용, 없으면 기본값 사용
  const speechBubbles = options.speechBubbles || defaultSpeechBubbles;

  // 사용자 정의 캐릭터 이미지
  const characterImages = options.characterImages || [];
 ...
 * 섹션 2: 십신 분석 생성
 */
export const generateSection2Content = (consultation, options = {}) => {
  // 기본 이미지 경로 또는 전달받은 이미지 사용
  const backgroundImage = options.backgroundImage || "/assets/images/consultation/section2/base.png";
  ...
  // 사용자 정의 캐릭터 이미지가 있으면 사용, 없으면 기본값 사용
  const characterImages = options.characterImages || [DEFAULT_CHARACTER];
 ...
 * @param {Object} options - 추가 옵션 (backgroundImage 등)
 */
export const generateSectionContent = (consultation, sectionNumber, options = {}) => {
  switch (sectionNumber) {
    case 1:
      return generateSection1Content(consultation, options);
    case 2:
      return generateSection2Content(consultation, options);
    case 3:

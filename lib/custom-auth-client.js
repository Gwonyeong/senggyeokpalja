...
export function clearAuthCookieClient() {
  ...
  try {
    ...
    supabaseCookiePatterns.forEach(pattern => {
      // 정확한 쿠키 이름으로 삭제
      document.cookie = `${pattern}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; SameSite=lax;`;
      document.cookie = `${pattern}=; max-age=0; path=/; SameSite=lax;`;

      // 하위 패턴들도 삭제 (.1, .2 등)
      for (let i = 0; i <= 5; i++) {
        document.cookie = `${pattern}.${i}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; SameSite=lax;`;
        document.cookie = `${pattern}.${i}=; max-age=0; path=/; SameSite=lax;`;
      }
    });
